...
yq '.version = env(MC_VERSION)' ./minecraft.yml > $WORKDIR/minecraft.yml
yq '.version = env(UNSUP_VERSION)' ./unsup.yml > $WORKDIR/unsup.yml
if [[ $MODLOADER == "forge" ]]; then
  yq '.version = env(FORGE_VERSION)' ./forge.yml > $WORKDIR/loader.yml
...
if [[ $LWJGL_MAJOR == "3" ]]; then
  yq '.version = env(LWJGL3_VERSION)' ./lwjgl3.yml > $WORKDIR/lwjgl.yml
elif [[ $LWJGL_MAJOR == "2" ]]; then
  yq '.version = env(LWJGL_VERSION)' ./lwjgl.yml > $WORKDIR/lwjgl.yml
else
...
yq -o json '.version = env(UNSUP_VERSION)' ./com.unascribed.unsup.yml > $WORKDIR/patches/com.unascribed.unsup.json
popd || exit 1
